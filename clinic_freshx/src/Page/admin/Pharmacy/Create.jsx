...
const Create = ({ isActive, onClose, onRefresh }) => {
 ...
 useEffect(() => {
  ...
  const fetchCostCenters = async () => {
   try {
     const response = await axios.get("/api/costcenter");
     setCostCenters(response.data.data);
   } catch (err) {
     message.error("Không thể tải danh sách trung tâm chi phí");
   }
 };
   ...
   >
     <Form layout="vertical">
       <Form.Item label="Mã nhà thuốc" required>
        ...
        <Form.Item label="Trung tâm chi phí">
         <Select
           placeholder="Chọn trung tâm chi phí"
           value={formData.costCenterId}
           onChange={(value) => handleChange("costCenterId", value)}
         >
